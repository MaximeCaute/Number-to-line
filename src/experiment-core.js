class ExperimentCore {
  ...
  createBlockInstructions(block){
    return ExperimentCore.createInstructionsTrial([
        this.localizer.getMessage([
          "NEW_LINE",
          block.modality == Modalities.VISUAL ? "VISUAL_MODALITY_DISPLAY" : "AUDITORY_MODALITY_DISPLAY"
  ...
  createTestTrial(blockData, numberFirstPart, numberSecondPart, value, numberType, isTraining = false, acceptOutOfBounds = true){
    if (!acceptOutOfBounds & value <= blockData.min - Number.EPSILON)
      return null;
    if (!acceptOutOfBounds & value >= blockData.max + Number.EPSILON)
      return null;

    // Ignore if it doesn't fall right.
    if (!MathUtils.isMultipleOfWithMargin(value, blockData.minorGraduationInterval))
      return null;
    ...
    return {
      ...
      minorGraduationInterval: blockData.minorGraduationInterval,
      intermediateGraduationInterval: blockData.intermediateGraduationInterval != undefined ? blockData.intermediateGraduationInterval : -1 ,
      majorGraduationInterval: blockData.majorGraduationInterval,
