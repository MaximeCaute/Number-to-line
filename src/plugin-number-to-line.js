  ...
  class NumberToLinePlugin {
    ...
    createLineElement(trial, target, displayElement){
      ...
      let line = NumberLine.createLinearLine(trial.rangeMinimum, trial.rangeMaximum,
        trial.majorGraduationInterval, trial.intermediateGraduationInterval, trial.minorGraduationInterval, trial.intermediateGraduationInterval > 0);
    ...
    createOutOfLineResponsePanel(trial, isLeft, target, displayElement){
      ...
      outOfLineResponsePanelDiv.id = isLeft ? NonNumericalResponses.SMALLER : NonNumericalResponses.GREATER;
    ...
    computeCorrectAnswer(trial){
      ...
      if (trial.rangeMinimum > target.value)
        return NonNumericalResponses.SMALLER;
      if (trial.rangeMaximum < target.value)
        return NonNumericalResponses.GREATER;
    ...
    giveCorrectiveFeedback(answer, trial){
      ...
      if (correctAnswer == NonNumericalResponses.SMALLER || correctAnswer == NonNumericalResponses.GREATER){
      ...
      if (correctAnswer == NonNumericalResponses.SMALLER){
        targetPosition = linePanelDivBoundingRect.left
          - document.getElementById(NonNumericalResponses.SMALLER).getBoundingClientRect().width / 2
      } else if (correctAnswer == NonNumericalResponses.GREATER) {
      ...
      let target = this.createTarget(trial.numberType, trial.numberFirstPart, trial.numberSecondPart);
      setTimeout(() => {
        if (trial.modality == Modalities.VERBAL){
          let audioSpecification = trial.audioVoice == undefined ? "" : `_${trial.audioVoice}`;
    ...
    tryHandleClick(responseElement, clickCoordinates, trial){
      ...
      if (typeof(correctAnswer) == typeof(0)){
        trial.correct = MathUtils.isEqualWithMargin(actualResponse.rounded, correctAnswer)
      } else {
    ...
    finishTrial(trial, trialData, timeout){
      ...
      // Add a small delay before ending the trial.
      // If there is corrective feedback, then there should be a longer delay to account for the feedback animation.
      setTimeout(function(){jsPsych.finishTrial(trialData)}, timeout + (trial.isTraining && !trial.correct ? 2*FEEDBACK_ANIMATION_DURATION : 0));
    }
    ...
    flagFractionError(trial){
      ...
      for (let inverted of [true, false]){
        for (let ten_value of [10, 1, 5, 9]) {
          if (MathUtils.isEqualWithMargin(trial.roundedResponse, fraction.decimalError(inverted, ten_value))){
            this.addFlag(trial, ErrorFlags.DECIMAL)
          }
        }
      }

      if(MathUtils.isEqualWithMargin(trial.roundedResponse, 1 / fraction.value))
        this.addFlag(trial, ErrorFlags.INVERTED);

      if (fraction.denominator == 2 && MathUtils.isEqualWithMargin(trial.roundedResponse, fraction.numerator + 0.5))
        this.addFlag(trial, ErrorFlags.MIXED)

      if (fraction.numerator == 2 && MathUtils.isEqualWithMargin(trial.roundedResponse, fraction.denominator + 0.5))
        this.addFlag(trial, ErrorFlags.MIXED)

      for (let graduation of [0.1, 1]){
        if (MathUtils.isEqualWithMargin(trial.roundedResponse, fraction.numerator * graduation))
      ...
      if (MathUtils.isEqualWithMargin(trial.roundedResponse, relativeResponse))
